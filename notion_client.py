...
def _update_config(api_key: str = "", transaction_db_id: str = "", reading_db_id: str = ""):
    """更新模块级配置（由 NotionClient.__init__ 调用）"""
    if api_key:
        _module_config["maton_api_key"] = api_key
    if transaction_db_id:
        _module_config["transaction_db_id"] = transaction_db_id
    if reading_db_id:
        _module_config["reading_db_id"] = reading_db_id
...
async def _module_async_request(method: str, endpoint: str, data: Optional[Dict] = None) -> Dict:
    """模块级异步请求（带重试）"""
    url = f"{GATEWAY}/{endpoint}"
    headers = {
        "Authorization": f"Bearer {_module_config.get('maton_api_key', '')}",
        "Notion-Version": "2022-06-28",
        "Content-Type": "application/json"
    }
    last_error = None
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.request(method, url, headers=headers, json=data, timeout=timeout) as resp:
                    if resp.status == 200:
                        return await resp.json()
                    elif resp.status == 429:
                        retry_after = resp.headers.get("Retry-After", "5")
    ...
    return {"error": f"请求失败: {last_error}"}


async def notion_get_pending_async(use_cache: bool = True) -> List[Dict]:
    """异步获取所有未完成任务（模块级函数，兼容旧代码）"""
    global _module_cache

    if use_cache and _module_cache["data"] and \
       (time.time() - _module_cache["timestamp"]) < _module_cache["ttl"]:
        return _module_cache["data"]

    results: List[Dict] = []
    if not _module_config.get("transaction_db_id") and not _module_config.get("reading_db_id"):
        return results

    # 查询事务库
    if _module_config.get("transaction_db_id"):
        cursor = None
        while True:
            data = {"page_size": 100}
            if cursor:
                data["start_cursor"] = cursor
            resp = await _module_async_request("POST", f"databases/{_module_config['transaction_db_id']}/query", data)
            if "error" in resp:
                break
            for page in resp.get("results", []):
                props = page.get("properties", {})
                ...
                date_prop = props.get("截止日", {}).get("date")
                ddl = date_prop.get("start") if date_prop else None
                results.append({"page_id": page["id"], "db_name": "事务", "title": title, "status": status, "ddl": ddl})
            if not resp.get("has_more"):
                break
            cursor = resp.get("next_cursor")

    # 查询阅读库
    if _module_config.get("reading_db_id"):
        cursor = None
        while True:
            data = {"page_size": 100}
            if cursor:
                data["start_cursor"] = cursor
            resp = await _module_async_request("POST", f"databases/{_module_config['reading_db_id']}/query", data)
            if "error" in resp:
                break
            for page in resp.get("results", []):
                props = page.get("properties", {})
                ...
                title_list = props.get("书目", {}).get("title", [])
                title = "".join([t.get("plain_text", "") for t in title_list]) or "(无标题)"
                results.append({"page_id": page["id"], "db_name": "阅读", "title": title, "status": status, "ddl": None})
            if not resp.get("has_more"):
                break
            cursor = resp.get("next_cursor")

    _module_cache["data"] = results
    _module_cache["timestamp"] = time.time()
    return results
