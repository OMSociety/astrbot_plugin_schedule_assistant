...
**Q: Apple 日历同步失败？**
- 确保 App 专用密码正确生成，且账号已开启两步验证
- 建议将 `apple_calendar_sync_interval` 设为 `15` 分钟及以上
...
├── schedule_store.py          # 数据持久化
├── notion_client.py           # Notion API 调用
├── apple_calendar.py          # Apple iCloud CalDAV 同步
├── constants.py               # 统一常量定义
